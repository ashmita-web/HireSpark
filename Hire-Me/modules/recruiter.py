...
def create():
    ...
    btn = Button(tab, text="Submit", font=(20), bg="#A00000", fg="#000000", command=submit_job)
...
def posted():
    ...
    search = Button(rt, text="Sort", font=('normal', 12, 'bold'),
                bg="#A00000", fg="#000000", command=lambda: sort_all(table))
    search.grid(row=0, column=3, padx=10, pady=10, ipadx=15)
    dlt = Button(rt, text="Delete", font=('normal', 12, 'bold'),
                bg="#A00000", fg="#000000", command=lambda: deletjob(table))
...
def app():
    ...
    search = Button(rt, text="Sort", font=('normal', 12, 'bold'),
                    bg="#8B0000", fg="#000000", command=lambda: sort_applicants(table))
...
def rec(root, email1):
    ...
    # Adjusting the LOGOUT button
    bn = Button(root, text="LOGOUT", font=('normal', 20),
            bg="#8B0000", fg="#000000",  # Change fg to black for better contrast
            activebackground="#ff4d4d",  # Change background color on hover
            relief="raised",               # Add a border effect
    ...
    lf.place(x=60, y=220)
    cj = Button(lf, text="Post a Job", font=('normal', 20),
            bg="#A00000", fg="#000000",
            activebackground="#a52a2a",  # Slightly lighter red on hover
            relief="raised",  # Add a border effect
            command=create)
    cj.grid(row=0, column=0, padx=80, pady=40)
    pj = Button(lf, text="Posted Jobs", font=('normal', 20),
            bg="#A00000", fg="#000000",
            activebackground="#a52a2a",  # Slightly lighter red on hover
            relief="raised",  # Add a border effect
            command=posted)
    pj.grid(row=1, column=0, padx=80, pady=40)
    ap = Button(lf, text="Applications", font=('normal', 20),
            bg="#A00000", fg="#000000",  # White text for contrast
            activebackground="#a52a2a",  # Slightly lighter red on hover
            relief="raised",  # Add a border effect
            command=app)
    ...
    bgr = Label(root, image=bgrf.load, bg="#00459A")
